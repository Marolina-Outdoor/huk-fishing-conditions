  ...
  class SlideShow extends HTMLElement {
    constructor() {
      ...
      if (slideshow.classList.contains('main-slideshow')) {
        ...
        // With fade mode + wrapAround, Flickity creates clones of every slide.
        // When there are only 2 slides, clone repositioning after each settle
        // briefly sets incorrect opacity on the real slides, causing a flash.
        if (fade && slideshow_slides.length < 3) {
          args.wrapAround = false;
        }
      }
      ...
      if (slideshow.classList.contains('products')) {
        ...
        function setSlideHeight(slider){
          const slides = slider.element.querySelectorAll('.carousel__slide');
          let maxHeight = 0;
          ...
          slides.forEach(slide => {
            ...
            const productCard = slide.querySelector('.product-card:not(.custom)');
            if (productCard) {
        ...
        //     setSlideHeight(flickity)});
          ...
          document.addEventListener('CollectionTab-change', (event) => {
      ...
      // ADA FIX P1-6: pause/resume button for WCAG 2.2.2 (moving content must be pausable)
      const pauseBtn = parent_section && parent_section.querySelector('.slideshow-pause-btn');
