...
const getCustomCss = async () => {
  const response = await fetch(frenzy_api_url+"/custom-css",{method: "GET",headers: {"Content-Type": "application/json","x-frenzy-authorization": frenzy_api_key,"X-Shop": x_shop}});
...
const getRecommendationAPIWithStructure = async (data_json, frenzy_recom_api, self,ApiName) => {
...
const getRecommendationProductsApi = async () => {
   document.querySelectorAll('.frenzy_recommendation_section').forEach(function(self){
     var classList = '';
     self.classList.forEach(function(classname, index){
      ...
      getRecommendationAPIWithStructure(data_json, frenzy_recom_api, classList,frenzyApiName);
   });
};
...
const getCrossSellCarouselApi = async () => {
    ...
    document.querySelectorAll('.frenzy_cross_sell_carousel_section').forEach(function(self){
        var classList = '';
        self.classList.forEach(function(classname, index){
          if (index == 0) {
            classList += classname;
...
const getMostClickedSkusApi = async () => {
    ...
    if (data.status == 200) {
      ...
      const dataResults = data?.data?.matching_products;
      const recommendationSetting = data?.data?.settings ? data?.data?.settings : data?.data?.most_click_recommendation;
      const recommendationCss = data?.data?.css ? data?.data?.css : data?.data?.most_click_recommendation_css;
      const dataGridHtml = data?.data?.recommendation_html;
      const frenzy_request_id = data?.data?.request_id;
      const dataConfig = data.data.config_id !== undefined ? data.data.config_id : data.config_id !== undefined ? data.config_id : null;
      const globalSetting = data?.data?.global_setting;
...
const getCartPageApi = async () => {
    ...
    if (data.status == 200) {
      ...
      const dataResults = data?.data?.matching_products ? data?.data?.matching_products :  data?.data?.products;
      const recommendationSetting = data?.data?.settings ? data?.data?.settings : data?.data?.cart_page;
      const recommendationCss = data?.data?.css ? data?.data?.css : data?.data?.cart_page_css;
      const dataGridHtml = data?.data?.recommendation_html;
      const frenzy_request_id = data?.data?.request_id;
      const dataConfig = data.data.config_id !== undefined ? data.data.config_id : data.config_id !== undefined ? data.config_id : null;
      const globalSetting = data?.data?.global_setting;
    ...
    get_frenzy_search_page_section.innerHTML = search_page_html;
     ...
     if(document.querySelectorAll('.filter_clear_icon').length > 0){
        document.querySelectorAll('.filter_clear_icon').forEach(function(this_click){
            ...
            if(document.querySelector('#'+this_click.getAttribute('for'))){
                document.querySelector('#'+this_click.getAttribute('for')).dispatchEvent(new Event('change', { 'bubbles': true }));
            }
            });
        });
        }
                ...
                if (settingData.filter_layout === '0') {
                    if (selectedTriggerType === "topbar") {
                        filter_order_Frenzy = (filter_order_Frenzy || []).map(
                            (x) => {
                                x.selected = x?.selected ? x.selected : 0;
                                return x;
                            }
                        );
    ...
    if (frenzy_layout_mode === "2") {
        ...
        setTimeout(function () {
          if (typeof frenzySwiperSliderCallBack == "function") {
              var swiper = frenzySwiperSliderCallBack("." + sectionClass ,frenzy_infinite_loop);
          }else{
            var swiper = new Swiper(
                "." + sectionClass + " .frenzy_recommendation_contain",
                {
                    ...
                    scrollbar: {
                        hide: false,
                        el: "." + sectionClass + " .swiper-scrollbar",
                        draggable: true,
                    },
                    paginationClickable: true,
                }
...
const getCollectionsPageApi = async () => {
    ...
    const frenzy_collection_page_collection_id = get_frenzy_collection_page_section.getAttribute("data-collection_id") ? get_frenzy_collection_page_section.getAttribute("data-collection_id") : frenzy_collection_id;
    ...
    filter_query.forEach((filter) => {
     ...
     if(collection_type != '' && collection_type != null || collection_vendor != '' && collection_vendor != null){
        var response = await fetch(
            authUrl_Frenzy + "/search?" + frenzy_default_collection_data_json + params,
            {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
...
const getCollectionsPageFilterChangeApi = async (
    ...
    if(collection_type != '' || collection_vendor != ''){
      var response = await fetch(
          authUrl_Frenzy + "/search?" + frenzy_collection_filter_data_json + params,
          {
              method: "GET",
              headers: {
                  "Content-Type": "application/json",
...
function getCollectionsPage(
    ...
    let featuredImage = settingData.image;
    let featuredText = settingData.text;
    let featuredImageLink = settingData.image_link;
    let featuredImagePosition = settingData.image_position;
    let featuredImageLength = settingData.image_length;
    let featuredImage2 = settingData.image_2;
    let featuredText2 = settingData.text_2;
    let featuredImageLink2 = settingData.image_link_2;
    let featuredImagePosition2 = settingData.image_position_2;
    let featuredImageLength2 = settingData.image_length_2;
    let featuredImage3 = settingData.image_3;
    let featuredText3 = settingData.text_3;
    let featuredImageLink3 = settingData.image_link_3;
    let featuredImagePosition3 = settingData.image_position_3;
    let featuredImageLength3 = settingData.image_length_3;
    let featuredImage4 = settingData.image_4;
    let featuredText4 = settingData.text_4;
    let featuredImageLink4 = settingData.image_link_4;
    let featuredImagePosition4 = settingData.image_position_4;
    let featuredImageLength4 = settingData.image_length_4;
    let featuredImage5 = settingData.image_5;
    let featuredText5 = settingData.text_5;
    let featuredImageLink5 = settingData.image_link_5;
    let featuredImagePosition5 = settingData.image_position_5;
    let featuredImageLength5 = settingData.image_length_5;
    let featuredImage6 = settingData.image_6;
    let featuredText6 = settingData.text_6;
    let featuredImageLink6 = settingData.image_link_6;
    let featuredImagePosition6 = settingData.image_position_6;
    let featuredImageLength6 = settingData.image_length_6;
    let featuredImage7 = settingData.image_7;
    let featuredText7 = settingData.text_7;
    let featuredImageLink7 = settingData.image_link_7;
    let featuredImagePosition7 = settingData.image_position_7;
    let featuredImageLength7 = settingData.image_length_7;
    let featuredImage8 = settingData.image_8;
    let featuredText8 = settingData.text_8;
    let featuredImageLink8 = settingData.image_link_8;
    let featuredImagePosition8 = settingData.image_position_8;
    let featuredImageLength8 = settingData.image_length_8;
    let featuredImage9 = settingData.image_9;
    let featuredText9 = settingData.text_9;
    let featuredImageLink9 = settingData.image_link_9;
    let featuredImagePosition9 = settingData.image_position_9;
    let featuredImageLength9 = settingData.image_length_9;
    let featuredImage10 = settingData.image_10;
    let featuredText10 = settingData.text_10;
    let featuredImageLink10 = settingData.image_link_10;
    let featuredImagePosition10 = settingData.image_position_10;
    let featuredImageLength10 = settingData.image_length_10;
    if (typeof FrenzyCollectionFeaturedSettingCallBack == "function") {
    var settings_schema = FrenzyCollectionFeaturedSettingCallBack();
        (settings_schema || []).map((x, i) => {
            if(i == '0'){
            featuredImage = x.image;
            featuredText = x.text;
            featuredImageLink = x.image_link;
            featuredImagePosition = x.image_position;
            featuredImageLength = x.image_length;
            }else if(i == '1'){
            featuredImage2 = x.image;
            featuredText2 = x.text;
            featuredImageLink2 = x.image_link;
            featuredImagePosition2 = x.image_position;
            featuredImageLength2 = x.image_length;
            }else if(i == '2'){
            featuredImage3 = x.image;
            featuredText3 = x.text;
            featuredImageLink3 = x.image_link;
            featuredImagePosition3 = x.image_position;
            featuredImageLength3 = x.image_length;
            }else if(i == '3'){
            featuredImage4 = x.image;
            featuredText4 = x.text;
            featuredImageLink4 = x.image_link;
            featuredImagePosition4 = x.image_position;
            featuredImageLength4 = x.image_length;
            }else if(i == '4'){
            featuredImage5 = x.image;
            featuredText5 = x.text;
            featuredImageLink5 = x.image_link;
            featuredImagePosition5 = x.image_position;
            featuredImageLength5 = x.image_length;
            }else if(i == '5'){
            featuredImage6 = x.image;
            featuredText6 = x.text;
            featuredImageLink6 = x.image_link;
            featuredImagePosition6 = x.image_position;
            featuredImageLength6 = x.image_length;
            }else if(i == '6'){
            featuredImage7 = x.image;
            featuredText7 = x.text;
            featuredImageLink7 = x.image_link;
            featuredImagePosition7 = x.image_position;
            featuredImageLength7 = x.image_length;
            }else if(i == '7'){
            featuredImage8 = x.image;
            featuredText8 = x.text;
            featuredImageLink8 = x.image_link;
            featuredImagePosition8 = x.image_position;
            featuredImageLength8 = x.image_length;
            }else if(i == '8'){
            featuredImage9 = x.image;
            featuredText9 = x.text;
            featuredImageLink9 = x.image_link;
            featuredImagePosition9 = x.image_position;
            featuredImageLength9 = x.image_length;
            }else if(i == '9'){
            featuredImage10 = x.image;
            featuredText10 = x.text;
            featuredImageLink10 = x.image_link;
            featuredImagePosition10 = x.image_position;
            featuredImageLength10 = x.image_length;
            }
        });
    }
    ...
    if(featuredImageVisibility == '1') {
        ...
        if(featuredImage3 != undefined || (featuredText3 != undefined && featuredText3 != '')) {
            if (1 == featuredImagePosition3) {
        ...
        if(featuredImage4 != undefined || (featuredText4 != undefined && featuredText4 != '')) {
            if (1 == featuredImagePosition4) {
        ...
        if(featuredImage5 != undefined || (featuredText5 != undefined && featuredText5 != '')) {
            if (1 == featuredImagePosition5) {
        ...
        if(featuredImage6 != undefined || (featuredText6 != undefined && featuredText6 != '')) {
            if (1 == featuredImagePosition6) {
        ...
        if(featuredImage7 != undefined || (featuredText7 != undefined && featuredText7 != '')) {
            if (1 == featuredImagePosition7) {
                searchPageProductList += getCollectionFeaturedImageElement(featuredImage7, featuredText7, featuredImageLink7, featuredImageLength7);
            }
        }

        if(featuredImage8 != undefined || (featuredText8 != undefined && featuredText8 != '')) {
            if (1 == featuredImagePosition8) {
                searchPageProductList += getCollectionFeaturedImageElement(featuredImage8, featuredText8, featuredImageLink8, featuredImageLength8);
            }
        }

        if(featuredImage9 != undefined || (featuredText9 != undefined && featuredText9 != '')) {
            if (1 == featuredImagePosition9) {
                searchPageProductList += getCollectionFeaturedImageElement(featuredImage9, featuredText9, featuredImageLink9, featuredImageLength9);
            }
        }

        if(featuredImage10 != undefined || (featuredText10 != undefined && featuredText10 != '')) {
            if (1 == featuredImagePosition10) {
                searchPageProductList += getCollectionFeaturedImageElement(featuredImage10, featuredText10, featuredImageLink10, featuredImageLength10);
            }
        }
    }
    ...
    (productData || []).map((x, i) => {
        ...
        if(featuredImageVisibility == '1') {
            if (featuredImageShowOnAllPages == '1') {
                if(featuredImage != undefined || (featuredText != undefined && featuredText != '')) {
                    if (featuredImagePosition != 1 && i == featuredImagePosition - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage, featuredText, featuredImageLink, featuredImageLength);
                    }
                }

                if(featuredImage2 != undefined || (featuredText2 != undefined && featuredText2 != '')) {
                    if (featuredImagePosition2 != 1 && i == featuredImagePosition2 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage2, featuredText2, featuredImageLink2, featuredImageLength2);
                    }
                }

                if(featuredImage3 != undefined || (featuredText3 != undefined && featuredText3 != '')) {
                    if (featuredImagePosition3 != 1 && i == featuredImagePosition3 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage3, featuredText3, featuredImageLink3, featuredImageLength3);
                    }
                }

                if(featuredImage4 != undefined || (featuredText4 != undefined && featuredText4 != '')) {
                    if (featuredImagePosition4 != 1 && i == featuredImagePosition4 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage4, featuredText4, featuredImageLink4, featuredImageLength4);
                    }
                }

                if(featuredImage5 != undefined || (featuredText5 != undefined && featuredText5 != '')) {
                    if (featuredImagePosition5 != 1 && i == featuredImagePosition5 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage5, featuredText5, featuredImageLink5, featuredImageLength5);
                    }
                }

                if(featuredImage6 != undefined || (featuredText6 != undefined && featuredText6 != '')) {
                    if (featuredImagePosition6 != 1 && i == featuredImagePosition6 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage6, featuredText6, featuredImageLink6, featuredImageLength6);
                    }
                }

                if(featuredImage7 != undefined || (featuredText7 != undefined && featuredText7 != '')) {
                    if (featuredImagePosition7 != 1 && i == featuredImagePosition7 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage7, featuredText7, featuredImageLink7, featuredImageLength7);
                    }
                }

                if(featuredImage8 != undefined || (featuredText8 != undefined && featuredText8 != '')) {
                    if (featuredImagePosition8 != 1 && i == featuredImagePosition8 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage8, featuredText8, featuredImageLink8, featuredImageLength8);
                    }
                }

                if(featuredImage9 != undefined || (featuredText9 != undefined && featuredText9 != '')) {
                    if (featuredImagePosition9 != 1 && i == featuredImagePosition9 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage9, featuredText9, featuredImageLink9, featuredImageLength9);
                    }
                }

                if(featuredImage10 != undefined || (featuredText10 != undefined && featuredText10 != '')) {
                    if (featuredImagePosition10 != 1 && i == featuredImagePosition10 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage10, featuredText10, featuredImageLink10, featuredImageLength10);
                    }
                }
            } else {
                if(featuredImage != undefined || (featuredText != undefined && featuredText != '')) {
                    if (previousProductCount + i == featuredImagePosition - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage, featuredText, featuredImageLink, featuredImageLength);
                    }
                }

                if(featuredImage2 != undefined || (featuredText2 != undefined && featuredText2 != '')) {
                    if (previousProductCount + i == featuredImagePosition2 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage2, featuredText2, featuredImageLink2, featuredImageLength2);
                    }
                }

                if(featuredImage3 != undefined || (featuredText3 != undefined && featuredText3 != '')) {
                    if (previousProductCount + i == featuredImagePosition3 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage3, featuredText3, featuredImageLink3, featuredImageLength3);
                    }
                }

                if(featuredImage4 != undefined || (featuredText4 != undefined && featuredText4 != '')) {
                    if (previousProductCount + i == featuredImagePosition4 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage4, featuredText4, featuredImageLink4, featuredImageLength4);
                    }
                }

                if(featuredImage5 != undefined || (featuredText5 != undefined && featuredText5 != '')) {
                    if (previousProductCount + i == featuredImagePosition5 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage5, featuredText5, featuredImageLink5, featuredImageLength5);
                    }
                }

                if(featuredImage6 != undefined || (featuredText6 != undefined && featuredText6 != '')) {
                    if (previousProductCount + i == featuredImagePosition6 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage6, featuredText6, featuredImageLink6, featuredImageLength6);
                    }
                }

                if(featuredImage7 != undefined || (featuredText7 != undefined && featuredText7 != '')) {
                    if (previousProductCount + i == featuredImagePosition7 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage7, featuredText7, featuredImageLink7, featuredImageLength7);
                    }
                }

                if(featuredImage8 != undefined || (featuredText8 != undefined && featuredText8 != '')) {
                    if (previousProductCount + i == featuredImagePosition8 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage8, featuredText8, featuredImageLink8, featuredImageLength8);
                    }
                }

                if(featuredImage9 != undefined || (featuredText9 != undefined && featuredText9 != '')) {
                    if (previousProductCount + i == featuredImagePosition9 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage9, featuredText9, featuredImageLink9, featuredImageLength9);
                    }
                }

                if(featuredImage10 != undefined || (featuredText10 != undefined && featuredText10 != '')) {
                    if (previousProductCount + i == featuredImagePosition10 - 2) {
                        searchPageProductList += getCollectionFeaturedImageElement(featuredImage10, featuredText10, featuredImageLink10, featuredImageLength10);
                    }
                }
            }
...
function frenzyAddScrollData(
    ...
    const featuredImage7 = dataSetting.image_7;
    const featuredText7 = dataSetting.text_7;
    const featuredImageLink7 = dataSetting.image_link_7;
    const featuredImagePosition7 = dataSetting.image_position_7;
    const featuredImageLength7 = dataSetting.image_length_7;
    const featuredImage8 = dataSetting.image_8;
    const featuredText8 = dataSetting.text_8;
    const featuredImageLink8 = dataSetting.image_link_8;
    const featuredImagePosition8 = dataSetting.image_position_8;
    const featuredImageLength8 = dataSetting.image_length_8;
    const featuredImage9 = dataSetting.image_9;
    const featuredText9 = dataSetting.text_9;
    const featuredImageLink9 = dataSetting.image_link_9;
    const featuredImagePosition9 = dataSetting.image_position_9;
    const featuredImageLength9 = dataSetting.image_length_9;
    const featuredImage10 = dataSetting.image_10;
    const featuredText10 = dataSetting.text_10;
    const featuredImageLink10 = dataSetting.image_link_10;
    const featuredImagePosition10 = dataSetting.image_position_10;
    const featuredImageLength10 = dataSetting.image_length_10;
    ...
    if(featuredImageVisibility == '1') {
        ...
        if(featuredImage7 != undefined || (featuredText7 != undefined && featuredText7 != '')) {
            if (1 == featuredImagePosition7) {
                frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage7, featuredText7, featuredImageLink7, featuredImageLength7);
            }
        }

        if(featuredImage8 != undefined || (featuredText8 != undefined && featuredText8 != '')) {
            if (1 == featuredImagePosition8) {
                frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage8, featuredText8, featuredImageLink8, featuredImageLength8);
            }
        }

        if(featuredImage9 != undefined || (featuredText9 != undefined && featuredText9 != '')) {
            if (1 == featuredImagePosition9) {
                frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage9, featuredText9, featuredImageLink9, featuredImageLength9);
            }
        }

        if(featuredImage10 != undefined || (featuredText10 != undefined && featuredText10 != '')) {
            if (1 == featuredImagePosition10) {
                frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage10, featuredText10, featuredImageLink10, featuredImageLength10);
            }
        }
    }
    ...
    (dataResults || []).map((x, i) => {
        ...
        if(featuredImageVisibility == '1') {
            if (featuredImageShowOnAllPages == '1') {
                ...
                if(featuredImage7 || (featuredText7 && featuredText7 != '')) {
                    if (featuredImagePosition7 != 1 && i == featuredImagePosition7 - 2) {
                        frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage7, featuredText7, featuredImageLink7, featuredImageLength7);
                    }
                }

                if(featuredImage8 || (featuredText8 && featuredText8 != '')) {
                    if (featuredImagePosition8 != 1 && i == featuredImagePosition8 - 2) {
                        frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage8, featuredText8, featuredImageLink8, featuredImageLength8);
                    }
                }

                if(featuredImage9 || (featuredText9 && featuredText9 != '')) {
                    if (featuredImagePosition9 != 1 && i == featuredImagePosition9 - 2) {
                        frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage9, featuredText9, featuredImageLink9, featuredImageLength9);
                    }
                }

                if(featuredImage10 || (featuredText10 && featuredText10 != '')) {
                    if (featuredImagePosition10 != 1 && i == featuredImagePosition10 - 2) {
                        frenzyAddScrollList += getCollectionFeaturedImageElement(featuredImage10, featuredText10, featuredImageLink10, featuredImageLength10);
                    }
                }
...
window.frenzyAutoSuggestApi = async (frenzy_query) => {
    ...
    if (frenzy_suggestion_page.query_as_first_suggestion == '1' || data.suggestions?.length > 0) {
        ...
        frenzy_suggest_html +=
            ...
            if (frenzy_query && frenzy_query != frenzy_suggestion_first_name && frenzy_suggestion_page.query_as_first_suggestion == '1') {
...
function getSuggestSearchData(
    ...
    if (frenzy_suggestion_name) {
        ...
        frenzy_suggestion_name +
        '" class="frenzy_sw_view_all_link"><div class="frenzy_sw_view_all_link_text"><span class="frenzy_sw_view_all_button_text_desktop">' +
        sw_view_all_button_text.replace(" for ", "") +
        '</span><span class="frenzy_sw_view_all_button_text_mobile">' +
        sw_view_all_button_text_mobile.replace(" for ", "") +
        '</span> <b class="frenzy_sw_view_all_link_search_term">' +
        frenzy_suggestion_name +
        "</b></div>" +
         "</a></div>";
    ...
    ($bundle_data.data.matching_products || []).map(
      ($bundle_product, $bundle_idx) => {
          if ($bundle_idx < frenzy_bundle_limit) {
              ...
              $frenzy_bundle_p_grid +=
              '<div class="frenzy_bundle_product_checkbox"><input data-handle="'+product_handle+'" id="bundle_product_'+$bundle_idx+'" type="checkbox" class="frenzy_bundle_product_checkbox_val" value="' +
                ...
                '" data-is_discount="' +
                is_discount +
                '" checked="checked"><label for="bundle_product_'+$bundle_idx+'"><span class="visually-hidden">'+$bundle_product?.org_product+' Bundle Checkbox</span></label></div>';
              $frenzy_bundle_p_grid +=
...
const $frenzy_bundle_section_html = function ($bundle_data) {
    ...
    let cssdata =
      ...
      settingCss.text_color +
      "}" +
      ".frenzy_bundle_id .frenzy_bundle_product_main_price.is_sale{color:#" +
      settingCss?.sale_price_color +
      "} " +
